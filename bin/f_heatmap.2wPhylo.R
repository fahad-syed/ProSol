            ...
            lwid = NULL, ...) 
{
  ...
  library(gplots)
  library(gtools) #required from invalid
  retval <- list()
  ...
  if (!(inherits(Colv, "dendrogram") || inherits(Colv, "phylo")) ) {
    cat(inherits(Colv, "dendrogram"),inherits(Colv, "phylo"),isTRUE(Colv),is.null(Colv),dendrogram,"here\n")
    if ((( !isTRUE(Colv)) || (is.null(Colv))) && (dendrogram %in% 
                                                 c("both", "column"))) {
      #cat("dendogram L69: ",dendrogram,class(Colv),"\n")
      ###############
  ...
  if (inherits(Rowv, "dendrogram")) {
    ...
    #rowInd <- order.dendrogram(ddr)
    rowInd <- order(Rowv,apply(x,1,median),apply(x,1,sd))
    if (nr != length(rowInd)) 
  ...
  if (inherits(tree,"phylo")) {
       #   cat("I have a column phylogram\n")
      colInd <- 1:length(tree$tip.label)
       }
  ###########
  ...
  if (dendrogram %in% c("both", "column")) {
    #cat("class(tree):",class(tree),"\n")
    if (inherits(tree,"phylo")) {
      require(ape)
                                        #      plot(tree,show.tip.label = TRUE)
     # cat("I am going to plot a col  phylogram\n")
                                        #old.par <- par()
  #x.lim is the thing to manipulate tree position
      treer <- plot(tree,direction='downwards',show.tip.label = FALSE,no.margin=F,tip.color=ColSideColors,x.lim=c(1.5,length(tree$tip.label)-.5))
      #print(treer)
                                        #par(old.par)
    } else {
